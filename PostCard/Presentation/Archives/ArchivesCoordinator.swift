...
class ArchivesCoordinator {
    ...
    func showCardOption(card: PostCard, delegate: CardOptionAlertDelegate?) {
        let cardOptionAlert = CardOptionAlert(card: card)
        cardOptionAlert.delegate = delegate
        navigationVontroller.present(cardOptionAlert, animated: true, completion: nil)
